...
# ── Label matching — substring-based, robust to TFDS version differences ─────
def _match_plantvillage(label: str):
    """Map any PlantVillage label to one of our 5 classes using substrings."""
    l = label.lower()
    if ("corn" in l or "maize" in l) and "rust" in l:
        return "maize_rust"
    if ("corn" in l or "maize" in l) and "blight" in l:
        return "maize_blight"
    if ("corn" in l or "maize" in l or "bean" in l) and "healthy" in l:
        return "healthy"
    if "bean" in l and ("angular" in l or "spot" in l):
        return "bean_spot"
    return None

def _match_cassava(label: str):
    """Map any Cassava label to one of our 5 classes."""
    l = label.lower()
    if "healthy" in l:
        return "healthy"
    if "mosaic" in l or "cgm" in l or "cmd" in l:
...
def generate_dataset():
    ...
    # Print first 5 labels so mismatches are easy to debug
    print("  PlantVillage label samples:", pv_labels[:5])

    for example in pv_ds:
        label_str = pv_labels[int(example["label"])]
        our_class = _match_plantvillage(label_str)
    ...
    print("  Cassava label samples:", cas_labels[:5])

    for example in cas_ds:
        label_str = cas_labels[int(example["label"])]
        our_class = _match_cassava(label_str)
        if our_class is None:
            continue
        if len(raw[our_class]) >= SAMPLES_PER_CLASS:
            continue
    ...
    for cls in CLASSES:
        imgs = raw[cls]
        if not imgs:
            print(f"⚠️  Class '{cls}' has 0 real images — generating purely synthetic stand-ins.")
            # Create simple color-tinted noise images as last-resort fallback
            base_colors = {
                "maize_rust":  [180, 100, 30],
                "maize_blight":[130, 130, 50],
                "bean_spot":   [100, 80,  50],
                "healthy":     [50,  170, 50],
                "cassava_mosaic":[150,180,70],
            }
            color = np.array(base_colors.get(cls, [128,128,128]), dtype=np.float32)
            rng = np.random.default_rng(SEED)
            for _ in range(30):
                noise = (rng.standard_normal((IMG_SIZE, IMG_SIZE, 3)) * 30 + color)
                imgs.append(np.clip(noise, 0, 255).astype(np.uint8))
...
# ── Export ONNX INT8 (replaces TFLite — Keras 3 / TFLite incompatible) ───────
def export_onnx_int8(onnx_fp32_path=MODEL_OUT, out_path="model_int8.onnx"):
    """
    Post-training static INT8 quantization of the FP32 ONNX model using
    onnxruntime.quantization. Avoids the Keras3 + TFLite LLVM crash entirely.
    """
    from onnxruntime.quantization import quantize_static, CalibrationDataReader, QuantType
    import onnxruntime as ort

    # ── Build a tiny calibration dataset from test images ────────────────────
    cal_images = []
    test_dir = pathlib.Path(DATA_DIR) / "test"
    for img_path in sorted(test_dir.rglob("*.jpg"))[:100]:
        img = cv2.imread(str(img_path))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
        cal_images.append(img.astype(np.float32)[np.newaxis] / 255.0)

    # Detect input name from the FP32 model
    sess = ort.InferenceSession(onnx_fp32_path, providers=["CPUExecutionProvider"])
    input_name = sess.get_inputs()[0].name

    class _CalReader(CalibrationDataReader):
        def __init__(self):
            self._it = iter(cal_images)
        def get_next(self):
            batch = next(self._it, None)
            return {input_name: batch} if batch is not None else None
    ...
    assert size_mb < 10, f"❌  Model too large: {size_mb:.2f} MB"
    return size_mb
